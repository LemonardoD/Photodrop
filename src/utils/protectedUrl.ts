...
import { getUserByRefToken, getUserByToken } from "../db/services/usersService";
...
export const accessPhotographersValidation: RequestHandler = async (req, res, next) => {
    ...
    const tokenInDB = await getUserByToken(req.headers.authorization.replace("Bearer ", ""));
    if (!tokenInDB.length) {
...
export const refreshPhotographersValidation: RequestHandler = async (req, res, next) => {
    ...
    const tokenInDB = await getUserByRefToken(req.headers.authorization.replace("Bearer ", ""));
    if (!tokenInDB.length) {
        return res.status(404).json({ 
            status: 404,
            message: "Wrong token."
